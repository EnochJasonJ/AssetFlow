...
function formatBookingItem(item) {
  const statusMap = {
    'UPCOMING': 'Upcoming',
    'ONGOING': 'Ongoing',
    'COMPLETED': 'Completed',
  ...
  let mappedStatus = statusMap[item.status] || item.status || 'Upcoming'
  if (mappedStatus !== 'Cancelled' && item.start_time && item.end_time) {
    const now = new Date()
  ...
  return {
    ...item,
    status: mappedStatus,
    resource_asset_id: item.asset_id || item.resource_asset_id,
    asset_name: item.asset?.name || item.asset_name || 'Unknown Asset',
    asset_tag: item.asset?.asset_tag || item.asset_tag || 'AF-0000',
    booked_by_name: item.user?.name || item.booked_by_name || 'User'
  }
}
...
export async function getBookings() {
  try {
    const res = await fetch('/api/v1/bookings', { headers: getAuthHeaders() })
    if (res.ok) {
      const data = await res.json()
      const list = data.data || data
      if (Array.isArray(list)) {
        return list.map(formatBookingItem)
      }
    }
  } catch (e) {
    console.warn('API getBookings failed:', e)
  }
...
export async function createBooking(bookingData, userName) {
  ...
  try {
    const res = await fetch('/api/v1/bookings', {
      ...
      body: JSON.stringify({
        asset_id: bookingData.resource_asset_id,
    ...
    if (res.ok) {
      const data = await res.json()
      return formatBookingItem(data)
    } else {
      const errJson = await res.json().catch(() => ({}))
      if (errJson.message) throw new Error(errJson.message)
    }
  } catch (e) {
    if (e.message && e.message.includes('conflicts')) throw e
    console.warn('API createBooking failed:', e)
  }
