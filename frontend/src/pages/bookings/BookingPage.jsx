...
export default function BookingPage() {
  ...
  const handleBookSubmit = async (e) => {
    ...
    const startISO = `${bookingForm.start_date}T${bookingForm.start_time}:00`
    const endISO = `${bookingForm.end_date}T${bookingForm.end_time}:00`
  ...
  const handleRescheduleSubmit = async (e) => {
    ...
    const startISO = `${rescheduleForm.start_date}T${rescheduleForm.start_time}:00`
    const endISO = `${rescheduleForm.end_date}T${rescheduleForm.end_time}:00`
  ...
  const resourceBookings = bookings.filter(b => {
    return b.resource_asset_id === selectedResource &&
      b.start_time.startsWith(selectedDate) &&
      b.status !== 'Cancelled'
  })

  // Check if a specific hour (e.g. 10:00) is occupied
  const getBookingForHour = (hour) => {
    return resourceBookings.find(b => {
      ...
      const endHour = bEndMin > 0 ? bEnd + 1 : bEnd
          ...
          {loading ? (
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '0.4rem' }}>
              {HOURS.map(hour => {
                ...
                return (
                  <div key={hour} style={{ display: 'flex', alignItems: 'center', gap: '1rem', padding: '0.5rem 0', borderBottom: '1px solid var(--border)' }}>
                    <div style={{ width: '80px', fontSize: '0.75rem', color: 'var(--text-secondary)', fontWeight: 500 }}>
                      {displayHour}
                    </div>
                    <div style={{ flex: 1, minHeight: '38px', borderRadius: '6px', background: isOccupied ? 'rgba(59, 130, 246, 0.08)' : 'rgba(255, 255, 255, 0.02)', border: isOccupied ? `1px solid ${statusColors[booking.status]}` : '1px dashed var(--border)', display: 'flex', alignItems: 'center', padding: '0 0.75rem' }}>
                      {isOccupied ? (
