...
| Database | PostgreSQL |
| ORM | **Prisma** (already in package.json — use it exclusively, no raw SQL) |
| Auth | JWT — **decision pending** (do not implement until team agrees; JWT is the likely choice) |
| Component Library | **Decision pending** — shadcn/ui or MUI (do not start mixing — team must agree first) |
