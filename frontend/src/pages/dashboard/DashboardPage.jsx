  ...
  async function fetchAll() {
    ...
    try {
      const [summaryRes, assetsRes] = await Promise.all([
        ...
        fetch('/api/v1/assets', { headers }).catch(() => null)
      ])

      let available = 0, allocated = 0, maintenance = 0, bookings = 0, transfers = 0, overdue = 0

      if (summaryRes?.ok) {
        const data = await summaryRes.json()
        const k = data.kpis || data.data?.kpis || data
        available = k.available_assets ?? 0
        allocated = k.allocated_assets ?? 0
        maintenance = k.under_maintenance_assets ?? 0
        bookings = k.active_bookings ?? 0
        transfers = k.pending_transfers ?? 0
        overdue = k.overdue_returns ?? 0
      } else if (assetsRes?.ok) {
        const data = await assetsRes.json()
        const list = data.data?.assets || data.data || []
        if (Array.isArray(list)) {
          available = list.filter(a => a.status === 'AVAILABLE' || a.status === 'Available').length
          allocated = list.filter(a => a.status === 'ALLOCATED' || a.status === 'Allocated').length
          maintenance = list.filter(a => a.status === 'UNDER_MAINTENANCE' || a.status === 'UnderMaintenance').length
        }
      }

      setKpis({ available, allocated, maintenance, bookings, transfers, overdue })
      setLoading(false)
      return
    } catch (err) {
      console.warn('Backend API fetch error on Dashboard:', err)
    }
