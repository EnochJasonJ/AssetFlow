...
export async function getCategories() {
  try {
    const res = await fetch('/api/v1/categories', { headers: getAuthHeaders() })
    if (res.ok) {
...
export async function getDepartments() {
  try {
    const res = await fetch('/api/v1/departments', { headers: getAuthHeaders() })
    if (res.ok) {
      const data = await res.json()
      const list = data.data || data
      if (Array.isArray(list)) return list
    ...
    console.warn('API getDepartments failed:', e)
  }
...
export function formatAssetItem(item) {
  if (!item) return item
  return {
    ...item,
    status: formatAssetStatus(item.status),
    category_name: item.category?.name || item.asset_categories?.name || item.category_name || 'Uncategorized'
  }
}

export async function getAssets() {
  try {
    const res = await fetch('/api/v1/assets', { headers: getAuthHeaders() })
    if (res.ok) {
      const data = await res.json()
      const list = data.data || data
      if (Array.isArray(list)) {
        return list.map(formatAssetItem)
      }
    }
  } catch (e) {
    console.warn('API getAssets failed:', e)
  }
  const assets = getStorage('af_assets', INITIAL_ASSETS)
  const categories = getStorage('af_categories', INITIAL_CATEGORIES)
  return assets.map(asset => formatAssetItem({
    ...asset,
  ...
  try {
    ...
    const res = await fetch('/api/v1/assets', {
      ...
      body: JSON.stringify({
        ...
        acquisition_cost: assetData.acquisition_cost || null,
        condition: (assetData.condition || 'EXCELLENT').toUpperCase(),
        location: assetData.location || null,
        photo_url: assetData.photo_url || null,
        is_bookable: !!assetData.is_bookable
      })
    })
    if (res.ok) {
      const data = await res.json()
      return formatAssetItem(data.data || data)
    }
  } catch (e) {
    console.warn('API registerAsset failed:', e)
  }
...
export async function getAssetHistories(assetId) {
  try {
    const res = await fetch(`/api/v1/assets/${assetId}/history`, { headers: getAuthHeaders() })
    if (res.ok) {
      const data = await res.json()
    ...
    console.warn('API getAssetHistories failed:', e)
  }
  if (isSupabaseValid()) {
    try {
